...
class FeatureExtractor:
    ...
    [1]  dst_port              — Destination port (normalized 0-1)
    [2]  bytes_sent            — Bytes sent (log-scaled)
    [3]  bytes_received        — Bytes received (log-scaled)
    ...
    def extract(self, event: NetworkFlowEvent) -> np.ndarray:
        ...
        features = [
            ...
            event.bytes_sent / max(event.bytes_sent + event.bytes_received, 1),
    ...
    def extract_batch(self, events: List[NetworkFlowEvent]) -> np.ndarray:
        """Convert a list of events to a feature matrix (N x 12)."""
